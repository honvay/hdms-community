 ...
 * by passing an empty object as the file: `var object = angular.extend({}, object1, object2)`.
...
function bootstrap(element, modules, config) {
  ...
  var doBootstrap = function() {
    ...
    if (config.debugInfoEnabled) {
      // Pushing so that this overrides `debugInfoEnabled` appSetting defined in user's `modules`.
      ...
      if (!eventFns) {
        ...
        if (type === 'mouseenter' || type === 'mouseleave') {
          ...
          jqLiteOn(element, MOUSE_EVENT_MAP[type], function(event) {
            ...
            // For mousenter/leave call the handler if related is outside the file.
 ...
 * # The array notation
 *
 * It is often desirable to inline Injected functions and that's when appSetting the `$inject` property
...
var $AnimateProvider = ['$provide', function($provide) {
   ...
   * When appSetting the classNameFilter value, animations will only be performed on elements
...
function $HttpProvider() {
   ...
   * Refer to {@link ng.$http#appSetting-http-headers $http} for documentation on
   * appSetting default headers.
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      if (responseType) {
        try {
          ...
          // known to throw when appSetting the value "json" as the response type. Other older
...
var locationPrototype = {
   ...
   * hash object.
   *
   * When called with a single argument the method acts as a setter, appSetting the `search` component
 ...
 * involve configuring the {@link ng.$sceDelegateProvider $sceDelegateProvider} instead by appSetting
 ...
 * by appSetting the 2nd parameter of the function to true).
 *
 ...
 * http://stackoverflow.com/a/472729)  However, appSetting img[src] does normalize the URL.
 * Unfortunately, appSetting img[src] to something like "javascript:foo" on IE throws an exception.
...
function FormController(element, attrs, $scope, $animate, $interpolate) {
   ...
   *
   * Setting a form controls back to their untouched state is often useful when appSetting the form
...
function addSetValidityMethod(context) {
  ...
  function setValidity(validationErrorKey, state, options) {
    ...
    // where appSetting/unsetting only increments/decrements the value,
