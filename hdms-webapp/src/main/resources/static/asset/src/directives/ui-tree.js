	...
	return {
		...
		template: function(element, scope) {
			var tpl = '<ul class="hc-tree">' +
				...
				'<a href="javascript:;" class="hc-lvl-{{node.$level}}" ng-class="{\'selected\':node.$selected || node.$checked, \'hidden\':node.$root}" ng-dblclick="treeCtrl.nodeDblClick(node,$event)" ng-click="treeCtrl.nodeClick(node)">' ;
